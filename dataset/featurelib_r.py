...
def calcFeat(Spec, feat):
    """compute spectral features"""

    if feat == "MagSpec":
        inpFeat = np.absolute(Spec)

    if feat == "PowSpec":
        inpFeat = np.power(np.absolute(Spec), 2)

    elif feat == "LogPow":
        pmin = 10**(-12)
        powSpec = np.power(np.absolute(Spec), 2)
...
def Feat2spec(Feat, phase, cfg):
	"""features to spectral"""

	if cfg == "MagSpec":
		Spec = Feat * np.exp(phase * 1j)

	if cfg == "PowSpec":
		Spec = np.sqrt(Feat) * np.exp(phase * 1j)

	elif cfg == "LogPow":
		Spec = np.power(10, 0.5 * Feat) * np.exp(phase * 1j)
...
def calcSpec(y, params, channel=None):
    ...
    # STFT parameters
    N_win = int(params["winlen"])
    if 'nfft' in params:
        N_fft = int(params['nfft'])
    else:
        N_fft = N_win
    N_hop = int((params["hopfrac"]))
    win = np.sqrt(np.hanning(N_win))
