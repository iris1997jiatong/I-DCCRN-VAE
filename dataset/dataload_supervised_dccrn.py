...
def build_dataloader(cfg, first_use_dataset):
    ...
    # Training dataset
    train_dataset = SpeechSequencesFull(noisy_file_list=noisy_train_file_list, clean_file_dir=clean_train_data_dir, 
                                        shuffle=shuffle, sample_mean=sample_mean, sample_std=sample_std, name=dataset_name, sr=fs, hop=hop, sequence_len=sequence_len,
                                        first_use=first_use_dataset, dataset_to='train')

    val_dataset = SpeechSequencesFull(noisy_file_list=noisy_val_file_list, clean_file_dir=clean_val_data_dir, 
                                    shuffle=shuffle, sample_mean=sample_mean, sample_std=sample_std, name=dataset_name, sr=fs, hop=hop, sequence_len=sequence_len,
                                    first_use=first_use_dataset, dataset_to='val')
...
class SpeechSequencesFull(data.Dataset):
    ...
    """
    def __init__(self, noisy_file_list, clean_file_dir, shuffle, sample_mean, sample_std, name='WSJ0', sr=16000, hop=100, sequence_len=100, first_use=True, dataset_to='train'):
    ...
    def __getitem__(self, index):
        ...
        # noisy_sample_lps_nor, frame_number, bin_number = extract_dns_lps(x, self.sample_mean, self.sample_std, self.wlen, self.nfft, self.hop, self.input_cfg)
        # clean_sample_lps_nor, frame_number, bin_number = extract_dns_lps(clean_x, self.sample_mean, self.sample_std, self.wlen, self.nfft, self.hop, self.input_cfg)
        # noise_sample_lps_nor, frame_number, bin_number = extract_dns_lps(noise_x, self.sample_mean, self.sample_std, self.wlen, self.nfft, self.hop, self.input_cfg)
        return x, clean_x
